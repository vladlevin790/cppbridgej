...
/**
 * Off-heap {@code int[]} wrapper for integer kernels.
 *
 ...
 * native call.</p>
 */
public final class NativeIntArray implements AutoCloseable {
    private final Arena arena;
    private final MemorySegment segment;
    private final int length;
    private boolean closed;

    private NativeIntArray(Arena arena, int length) {
        if (length < 0) {
            throw new IllegalArgumentException("length must be >= 0");
        }
        this.arena = Objects.requireNonNull(arena, "arena");
        this.length = length;
        this.segment = arena.allocate(ValueLayout.JAVA_INT.byteSize() * length, ValueLayout.JAVA_INT.byteAlignment());
    }
     ...
     */
    public static NativeIntArray allocate(int length) {
        return new NativeIntArray(Arena.ofConfined(), length);
    }
     ...
     */
    public static NativeIntArray copyOf(int[] values) {
        Objects.requireNonNull(values, "values");
        NativeIntArray array = allocate(values.length);
    ...
    private void ensureOpen() {
        if (closed) {
            throw new CppBridgeException("NativeIntArray is already closed");
        }
    }
